...
const PACK_META = {
  ...
  last30days: {
    ...
    version: '3.21.1',
    note: 'Recency radar. Treat hits as leads, not evidence. Discover on demand.'
  },
  'agent-deep-research': {
    title: 'agent-deep-research',
    source: '24601/agent-deep-research',
    version: '2.1.3',
    note: 'One deep-research skill (not Weizhena or other forks). Needs uv + a Gemini key at runtime; do not commit keys.'
  },
  hallmark: {
    title: 'Hallmark',
    source: 'nutlope/hallmark',
    version: '1.1.0',
    note: 'UI taste / anti-slop gate. Discover on demand.'
  },
  impeccable: {
    title: 'Impeccable',
    source: 'pbakaus/impeccable',
    version: '4.1.2',
    note: 'Frontend craft skill (Apache-2.0). Vendored from the upstream Cursor skill tree.'
  },
  openspec: {
    title: 'OpenSpec (project-local)',
    source: 'Fission-AI/OpenSpec',
    version: '1.11.0',
    note: 'Default spec/change layer. Generated by openspec init; Spec Kit stays for heavyweight greenfield.'
  },
  graphify: {
    title: 'Graphify',
    source: 'graphifyy',
    href: 'https://pypi.org/project/graphifyy/',
    label: 'graphifyy (PyPI)',
    version: '0.9.51',
    note: 'Optional knowledge graph. Command `graphify`. Indexing is not part of Cloud Agent boot.'
  }
};
...
function sourceLink(meta) {
  if (meta.href) {
    const label = meta.label || meta.source;
    return `[${label}](${meta.href})`;
  }
...
md += 'gstack also vendors pack `bin/` and `setup`. Cursor slash commands come from flattened ';
md += '`.cursor/skills/<name>/` links for **every** vendored pack (see `scripts/link-agent-skills.sh`). ';
md += 'That index is discovery, not always-on personality. ';
...
md += '| Spec / change (default) | OpenSpec (`openspec/` + `/opsx-*` commands). Spec Kit is heavyweight greenfield only. |\n';
md += '| AWS / Cloudflare / Supabase | `aws/*`, `cloudflare/*`, `supabase/*` when that stack is in play |\n';
md += '| Recency radar / deep research | `last30days/last30days` (leads, not evidence), `agent-deep-research` |\n';
md += '| UI taste / design engineering | `hallmark/hallmark` (taste), `impeccable/impeccable` (design engineering) |\n';
md += '| Knowledge graph (optional) | `graphify/graphify` once `graphify-out/graph.json` exists |\n';
md += '| Library docs / this-repo intel | Context7 MCP / Serena MCP ([docs/agent-skill-packs.md](agent-skill-packs.md)) |\n';
md += '| Compound after ship | `compound-engineering/ce-compound` (not on the same task as gstack, Superpowers, or pstack) |\n\n';
md += 'Pick **one** execution engine per task: **pstack XOR Superpowers XOR gstack**. ';
md += 'OpenSpec is the default WHAT/change layer; Spec Kit is heavyweight greenfield. ';
md += 'Compound Engineering is the learn/compound layer after a run. Do not run those conductors together.\n\n';
